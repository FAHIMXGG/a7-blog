...
import { blogUpdateSchema, type BlogUpdateFormValues, type BlogUpdateValues } from "@/app/lib/blog-validators"
...
export default function BlogEditForm({ post }: { post: any }) {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting }, setValue, watch } =
    useForm<BlogUpdateFormValues>({
      ...
      defaultValues: {
        ...
        thumbnailUrl: post.thumbnailUrl ?? null, // <— NEW
      },
    })
  ...
  const onSubmit = async (values: BlogUpdateFormValues) => {
    try {
      const res = await fetch(`/api/blogs/${post._id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(values as BlogUpdateValues),
      })
